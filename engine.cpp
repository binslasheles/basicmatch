...
void Engine::match(order_info_t& o, Levels& levels, uint64_t& txn_id, std::vector<order_action_t>& results) {
    ...
    while (it!=end) {
        ...
        while (o.qty_) {
            ...
            fill(resting, o, level_price, txn_id++, results);
            level.pop_front();
...
void Engine::handle_cancel(uint32_t order_id, std::vector<order_action_t>& results) {
    ...
    if (it == orders_.end())
        ...
        cancel_cb_(o.symbol_, book.txn_id_++, o.qty_, o.price_);
...
void Engine::handle_submit(order_info_t& o, std::vector<order_action_t>& results) {
    if( (orders_.find(o.id_) != orders_.end()) || (completed_orders_.find(o.id_) != completed_orders_.end()))
        ...
        std::vector<order_action_t>& results)
{
    ...
    fill_cb_(large.symbol_, txn_id, fill_qty, fill_price);//no aggressor...
}
